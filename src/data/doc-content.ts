...
const pageSeeds: Record<string, ContentSeed> = {
	...
	'agent-teams': {
		...
		explain: ['你就把它想成包工头带施工队。你平时主要跟 lead 说清楚要干什么，lead 再把活拆给不同队友，最后把结果收回来。', '它跟子代理最大的差别，是这里真有好几个人同时在跑，各人有各人的上下文和进度，所以并行更强，协调也更重要。'],
		practice: ['只有任务真能拆成几路并行时，团队才值。小活硬拉一队人，只会把商量、等待和花费一起拉高。', '最稳的拆法通常是“谁去查、谁去改、谁去验”先分清，尽量别让多人同时冲同一块代码。'],
		steps: [{ title: '适合团队的活', body: '下面这几种活，拉团队一起上通常更划算。', code: `1. 多模块并行排查\n2. 一边实现一边补测试\n3. 代码、文档、验收报告分路推进`, kind: 'code' }]
	},
	plugins: {
